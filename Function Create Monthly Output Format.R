CreateMonthlyOutputFormat <- function(FC_Monthly, monthlyData){
  #this function converts the input of type Forecast to a format used for presentation
  
  # convert Forecast to format used in output
  FC_Monthly_TS           <- FC_Monthly$mean # Convert forecast to TS
  FC_Monthly_DF           <- data.frame(energy = c(FC_Monthly_TS), 
                                        month = c(time(FC_Monthly_TS))) # Convert TD to DF
  ...
  FC_Monthly_DF$monthname <- as.character(month(ymd(010101) + months(FC_Monthly_DF$month-1),
                                          label=TRUE,
                                          abbr=TRUE))
  FC_Monthly_DF$MonthDays <- with(FC_Monthly_DF, 
                                  ifelse(month %in% c(1, 3, 5, 7, 8, 10, 12),
                                         31,
  ...
  CurrentMonthUsed      <- as.numeric(MonthlyActual %>% 
                                        ...
                                        ungroup() %>%
                                        select(monthCost ))
  MonthlyPredict$monthCost <- with(MonthlyPredict, 
                                   ifelse(month==11, 
                                          monthCost - CurrentMonthUsed, 
                                          monthCost)
                                   )
