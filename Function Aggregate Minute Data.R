AggregateMinuteData <- function(FullData, Agg_interval) {
  ...
  if (Agg_interval == "Monthly") {
    Agg_data <- arrange(FullData %>%   
                        ...
                        yearmonth)
    # add number of days in calender-month
    Agg_data$MonthDays <- with(Agg_data, 
                               ifelse(month %in% c(1, 3, 5, 7, 8, 10, 21),
                                      31,
                                      ifelse(month %in% c(4,6,9,11),
                                            30,
                                            ifelse(yearmonth==200802,
                                                   29,
                                                   28)
                                            )
                                      )
                               )
    # add number of days data present in that month
    ...
    Agg_data$monthname    <- as.character(month(ymd(010101) + months(Agg_data$month-1),
                                          label=TRUE,
                                          abbr=TRUE))
    # convert energy usage to cost in euro
